declare module 'svelte' {
	...
	const EFFECT: number;
	const PRE_EFFECT: number;
	const RENDER_EFFECT: number;
	export function unstate<T extends StateObject>(value: T): Record<string | symbol, any>;


	const STATE_SYMBOL: unique symbol;
	type Metadata = {
	...
	type StateObject = Record<string | symbol, any> & {
		[STATE_SYMBOL]: Metadata;
	};
	...
	type ComputationSignal<V = unknown> = {
	...
	type Signal<V = unknown> = SourceSignal<V> | ComputationSignal<V>;

	type EffectSignal = ComputationSignal<null | (() => void)>;

	type EqualsFunctions<T = any> = (a: T, v: T) => boolean;
